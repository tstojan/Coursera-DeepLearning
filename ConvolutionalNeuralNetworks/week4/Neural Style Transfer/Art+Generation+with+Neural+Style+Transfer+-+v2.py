...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(a_C, [n_H*n_W*n_C, 1])
    a_G_unrolled = tf.reshape(a_G, [n_H*n_W*n_C, 1])
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GS, GG)))/(4.0*n_H*n_W*n_C*n_H*n_W*n_C)
